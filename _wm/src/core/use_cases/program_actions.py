...
class ProgramActions:
    ...
    def run(self, action: str, program: ProgramName, backup: bool = False) -> None:
        ...
        if action == "install-requirement":
            self._package_repo.install(program, cfg.packages, manager=cfg.package_manager)
            return

        if action == "uninstall-requirement":
            self._package_repo.uninstall(program, cfg.packages, manager=cfg.package_manager)
            return
