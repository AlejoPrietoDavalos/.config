...
class RangerConfigRepository(ProgramBuildConfigRepository):
    def build_config(self) -> ProgramConfig:
        program_root = path_wm_programs / "ranger"
        return ProgramConfig(
            name="ranger",
            files_dir=program_root / "files",
            target_dir=path_dotfiles / "ranger",
            packages=Packages(packages=[PackageSpec(manager="pacman", names=["ranger"])]),
        )
